...
class BenchmarkModel(Model):
    ...
    def propagate(cls, n, x):
        return x / 2 + 25 * x / (1 + x * x) + 8 * np.cos(1.2 * n) + \
            np.sqrt(10) * np.random.randn()
    ...
    def tranpdf(cls, n, x, pre_x):
        return norm.pdf(x, pre_x / 2 + 25 * pre_x / (1 + pre_x * pre_x)
                        + 8 * np.cos(1.2 * n), np.sqrt(10))
...
if __name__ == '__main__':
    BenchmarkModel.generatedata(100, 1.0, 'benchmarkob.txt',
                                'benchmarkstate.txt')
